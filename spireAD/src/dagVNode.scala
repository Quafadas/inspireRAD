...
trait VNode[F[_], T](forZeroVal: F[T])(using
  ...
  def backward[N <: VNode[?, T]]: Unit
end VNode

// case class VConstNode[F[_], T](value: F[T])(using
//     vf: VectorisedField[F, T],
//     vt: VectorisedTrig[F, T]
// ) extends VNode[F, T](value):
//   // var grad: F[T] = vf.zero(value)

//   override def toString(): String =
//     s"const \n v:$value g: $grad \n (_id: ${id.toString().takeRight(4)})"

//   override def backward[N <: VNode[?, T]](using
...
//   override def backward[N <: VNode[F, T]](using
//       f: VectorisedField[F, T],
//       t: VectorisedTrig[F, T]
//   ): Unit =
...
//       */
//     ???
//     // val n: VNode[F, T] = td.dag.getNode(dep).asInstanceOf[VNode[F, T]]
